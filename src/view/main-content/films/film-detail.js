export const createFilmDetails = (film) => {
  ...
  const createGenres = (genre) => {
    const createTemplate = document.createDocumentFragment();
    genre.forEach((item) => {
       createTemplate.append(`<span class="film-details__genre">${item}</span>`);
    });

    return createTemplate.textContent;
  };

  const createComment = (comments) => {
    const createTemplate = document.createDocumentFragment();
    comments.forEach((item) => {
       createTemplate.append(`
        <li class="film-details__comment">
            <span class="film-details__comment-emoji">
              <img src="./images/emoji/${item.emotion}" width="55" height="55" alt="emoji-smile">
            </span>
            <div>
              <p class="film-details__comment-text">${item.message}</p>
